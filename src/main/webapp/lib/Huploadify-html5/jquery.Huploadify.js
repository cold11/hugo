		...
		this.each(function(index, element){
			...
			var uploadManager = {
				...
				init : function(){
					...
					inputStr += '"/>';
					inputStr += '<a id="file_upload_'+instanceNumber+'-button" href="javascript:void(0)" class="'+option.btnClass+'">';
				...
				_renderFile : function(file){
                    var count = uploadManager.filteredFiles.length;
                    if(count>opts.queueSizeLimit){
                        alert("一次只能上传"+opts.queueSizeLimit+"个文件,请删除再上传!");
                        return;
                    }
