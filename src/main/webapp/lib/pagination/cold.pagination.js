/**
 * name: cold.pagination
 * Version: 0.0.1
 */
angular.module('cold.pagination', []).directive('coldPagination',[function(){
    return {
        restrict: 'EA',
        template: '<div class="pages" ng-show="conf.totalItems > 0">' +
            '<ul class="pagination pagination-sm m-t-none m-b-none">' +
            '<li ng-class="{disabled: conf.currentPage == 1}"><a href="javascript:void(0);" ng-click="prevPage()" title="上一页">&laquo;</a></li>' +
            '<li ng-repeat="item in pageList track by $index" ng-class="{active: item == conf.currentPage, separate: item == \'...\'}" >' +
            '<a href="javascript:void(0);" ng-click="changeCurrentPage(item)">{{ item }}</a>' +
            '</li>' +
            '<li ng-class="{disabled: conf.currentPage == conf.numberOfPages}"><a href="javascript:void(0);" ng-click="nextPage()" title="下一页">&raquo;</a></li>' +
            '</ul>' +
            '　第<input type="text" ng-model="jumpPageNum"  ng-keyup="jumpToPage($event)" class="form-control"/>页/共{{conf.numberOfPages}}页 ' +
